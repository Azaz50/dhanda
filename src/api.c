...
api_party_add(struct http_request *req)
{
	...
	ret = util_http_json_error_response(req, item, "amount", validate_amount_kore);
	if (ret == -1) goto cleanup;
...
api_txn_add(struct http_request *req)
{
	...
	ret = util_http_json_error_response(req, item, "party_id", validate_pid);
	if (ret == -1) goto cleanup;
	t.party_id = (int) item->data.u64;

	item = kore_json_find_integer(json.root, "amount");
	ret = util_http_json_error_response(req, item, "amount", validate_amount);
	if (ret == -1) goto cleanup;
	t.amount = (int) item->data.u64;

	item = kore_json_find_integer(json.root, "type");
	ret = util_http_json_error_response(req, item, "type", validate_type);
	if (ret == -1) goto cleanup;
